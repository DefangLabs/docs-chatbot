...
def fetch_intercom_conversation(conversation_id):
    ...
    if response.status_code != 200:
        logger.error(
            f"Failed to fetch conversation {conversation_id} from Intercom; status code: {response.status_code}, response: {response.text}"
        )
        return jsonify(
            {"error": "Failed to fetch conversation from Intercom"}
...
def get_user_query(response, conversation_id, topic):
    ...
    if topic == "conversation.user.replied":
        ...
        logger.info(
            f"Extracted {len(result)} parts from conversation {conversation_id}"
        )
...
def extract_conversation_parts(response):
    ...
    for part in parts:
        ...
        extracted_parts.append(
            {"body": body, "author": author, "created_at": created_at}
        )
...
def extract_latest_user_messages(conversation_parts):
    ...
    if last_non_user_idx is not None:
        last_user_entries = [
            part
            for part in conversation_parts[last_non_user_idx + 1 :]
...
def set_conversation_human_replied(conversation_id, redis_client):
    try:
        ...
        redis_client.set(
            conversation_id, "1", ex=60 * 60 * 24
        )  # Set TTL expiration to 1 day
        logger.info(
            f"Added conversation_id {conversation_id} to Redis set admin_replied_conversations"
        )
...
def post_intercom_reply(conversation_id, response_text):
    ...
    logger.info(
        f"Posted reply to Intercom; response status code: {response.status_code}"
    )
...
def answer_intercom_conversation(rag, conversation_id, topic):
    ...
    llm_response = "".join(
        generate(rag, user_query, "Intercom Conversation", anon_hash)
    )
    llm_response = (
        llm_response + " 🤖"
    )  # Add a marker to indicate the end of the response
